...
// //added a func to return predecessor of the given m=node
...
//       return tmp;
//     }else{
//       return getPredecessor(tmp->right);
//     }
//   }
// }
...
bool delete_node(Node*& root, int key) {
  ...
  if (target->left == NULL) {
    ...
    } else// case 0: target is the root and has only right child
           root = target->right;
   // free target
   delete target;
   return true;
  ...
  // case 3: target has two children
     ...
     */
     ...
     // set parent's child pointer
      Node* predecessor = target->left;//tmp predecessor for replacing current target's key
     ...
     //  }else{
     //      predecessor= maxNode(predecessor->right);
     //  }
     // }
    ...
    // }
     ...
     target->key = predecessor->key;//replacement
     Node* pre_parent = find_parent(root, predecessor);
   ...
   */
   ...
   if(x != NULL){
     if((x->left == NULL) && (x->right == NULL)){
        ...
        // h = 1 + (height(x->left) > height(x->right) ? height(x->left) : height(x->right));
        // return h;
...
int depth( Node* root, Node* x ) {
   ...
   */
   ...
   if(x == root || root == NULL){
      ...
      if(x->key < root->key){
         d = 1 + depth(root->left, x);
         return d;
      }else{
         d = 1 + depth(root->right,x);
         return d;
      }
   }
    return 0;
